...
namespace item
{
	...
	struct Item_shared : public Component_base
	{
		...
		Item_shared(std::string name = ""s, std::string description = ""s, int price = 0, float weight = 0.0f) noexcept : Component_base{ Component_type::Item_shared }, m_price{ price }, m_weight{ weight } {}
		std::string m_name;
	...
	struct Projectile : public Component_base
	{
		...
		Projectile(Point point_type = Point::Default, int damage_bonus = 0, int range_multiplier = 0) noexcept : Component_base{ Component_type::Projectile }, m_point_type{ point_type }, m_damage_bonus{ damage_bonus }, m_range_multiplier{ range_multiplier } {}
		Point m_point_type;
